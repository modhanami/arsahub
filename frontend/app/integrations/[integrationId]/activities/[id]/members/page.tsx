...
export default function Page({ params, searchParams }: Props) {
  ...
  function handleClick(member: MemberResponse) {
    router.push(
      `/integrations/${params.integrationId}/activities/${params.id}/members?userId=${member.username}`
    );
    ...
    <DashboardShell>
      ...
      <div className="flex gap-4">
        ...
        <div className="w-1/2">
          {searchParams.userId && (
            <iframe
              src={`/embed/integrations/${params.integrationId}/activities/${params.id}/profile?userId=${searchParams.userId}`}
              width="100%"
              height="100%"
