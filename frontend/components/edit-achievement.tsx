...
export const achievementEditSchema = z.object({
  title: z
    .string({
...
type FormData = z.infer<typeof achievementEditSchema>;
...
export function AchievementEditForm({
  ...
  const form = useForm<FormData>({
    resolver: zodResolver(achievementEditSchema),
    defaultValues: {
  ...
  const updateMutation = useEditAchievement();
  const [preview, setPreview] = React.useState<string | null>(
    achievement.imageKey ? getImageUrlFromKey(achievement.imageKey) : null,
  );
  ...
  async function onSubmit(values: FormData) {
    const request: EditAchievementParams = {
  ...
  return (
    <>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <CardTitle>Update achievement</CardTitle>
        </DialogHeader>
        <Form {...form}>
