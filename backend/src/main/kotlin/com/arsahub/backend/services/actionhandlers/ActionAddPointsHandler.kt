...
class ActionAddPointsHandler(private val userActivityRepository: UserActivityRepository) : ActionHandler {
    override fun handleAction(rule: Rule, member: AppUserActivity): ActionResult {
        ...
        println("User ${member.appUser?.displayName}` (${member.appUser?.userId}) received `$points` points for activity `${rule.activity?.title}` (${rule.activity?.activityId}) from rule `${rule.title}` (${rule.id}), previous points: $previousPoints, new points: $newPoints")
